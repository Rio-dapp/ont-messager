        ...
        if func.parameters[i]['type'] == "String":
            ...
            try:
                ...
                if func_map["param_list"][i].startswith("A"):
                    addr = Address.b58decode(func_map["param_list"][i]).to_bytearray()
                else:
                    addr = Address(bytearray.fromhex(func_map["param_list"][i])).to_bytearray()
                params.append(addr)
            except Exception as e:
                    ...
                    for p in dict(param).values():
                        if type(p) is str:
                            ...
                            if str(p).split(":")[0] == "String":
                                ...
                                try:
                                    temp.append(Address.b58decode(str(p).split(":")[1]).to_bytearray())
                                except Exception as e:
